	...
	switch ($aksi){
		...
		case 'ubah':
			if(isset($_POST['passwordTB']) and $_POST['passwordTB']!=''){
				$pass = ",password = '".md5(mysqli_real_escape_string($_POST['passwordTB']))."'";
			}else{
				$pass="";
			}
			$sql = "update  user set 	username 	= '".mysqli_real_escape_string($_POST['usernameTB'])."',
		...
		#tambah  ==============================================================================================
		case 'tambah':
			$sql = "INSERT into user set 	username= '".trim(mysqli_real_escape_string($_POST['usernameTB']))."',
											password= '".md5(mysqli_real_escape_string($_POST['passwordTB']))."',
											level	= '$_POST[levelTB]'";
			$exe	= mysqli_query($con,$sql);
			if($exe){
				echo '{"status":"sukses"}';
