var dir = 'server/p_mkwarda.php';
	...
	function loadData(){
		...
		$.ajax({
			...
			data: 'aksi=tampil',
			success:function(data){
				$('#loadarea').html('<i class="icon-th-list"></i> DAFTAR KWARDA');
		...
		$.ajax({
			...
			success:function(data){
				if(data.status=='gagal'){
					...
					$('#loadarea').html('<i class="icon-edit"></i> UBAH KWARDA').fadeIn();
					$('#i_kegPN').toggle(1000);
					$('#v_kegPN').toggle();
					$('#viewBC').toggle();
					$('#addBC').toggle();
	...
	$(document).ready(function(){
		...
		$('#addBC').click(function(){
			...
			$('#v_kegPN').toggle();
			$('#viewBC').toggle();
			$('#loadarea').html('<i class="icon-plus"></i> TAMBAH KWARDA').fadeIn();
		});
