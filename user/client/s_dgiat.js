var dir='server/p_dgiat.php';
	...
	function loadData(){
	 ...
	 $.ajax({
			...
			data: datax+cari,
			success:function(data){
				$('#loadarea').html('<i class="icon-th-list"></i> DAFTAR KEPRAMUKAAN');
	...
	$(document).ready(function(){
		...
		$('#cetakBC').on('click',function(){
			var href='report/r_dgiat.php'
					+'?tipe=pdf'
					+'&tingkat='+$('#tingkatTS').val()
					+'&lokasi='+$('#lokasiTS').val()
					+'&mgolongan='+$('#mgolonganTS').val()
					+'&kategori='+$('#kategoriTS').val()
					+'&status='+$('#statusTS').val()
					+'&ket='+$('#ketTS').val()
		...
		$('#mgolonganTS').on('change',loadData);
		$('#tingkatTS').on('change',loadData);
		$('#kategoriTS').on('change',loadData);
		$('#statusTS').on('change',loadData);
