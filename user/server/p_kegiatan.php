	...
	#save : sisa poin sebelumnya ============================================================================================================
			case 'saveSisa':
				if(isset($_POST['sisaTB'])){
					...
					$sisa = trim(mysqli_real_escape_string($_POST['sisaTB']));
				...
				if($exe){
					...
					if($bukegArr!=NULL){
						echo '{"kondisi":"sukses",
							...
							// "ket":"'.mysqli_real_escape_string(stripslashes(htmlspecialchars($res['ket'],ENT_QUOTES))).'",
			...
			case 'tampil' :
				$sql = 'SELECT
						...
						WHERE
							...
							s.id_katkeg ='.$_GET['menu'].'
							
						ORDER BY 
							dk.tglinput desc, dk.status asc';
				// print_r($sql);exit();
