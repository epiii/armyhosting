		...
		case 'ubah':
			...
			# udpate : mlogin  -------------------
				if(isset($_POST['passBTB2']) && !empty($_POST['passBTB2'])){
					$sql1	='UPDATE mlogin set paswot="'.md5(trim(mysql_real_escape_string($_POST['passBTB2']))).'" where id_mlogin='.$_SESSION['id_mloginp'];
			...
			//upload image
				if(isset($_POST['fileadd'])){
					...
					$r=mysql_fetch_assoc(mysql_query('SELECT foto from manggota WHERE id_mlogin='.$_SESSION['id_mloginp']));
			...
			#add / update : malamat & mdonatur
				$sal=' 	malamat		= "'.trim(mysql_real_escape_string($_POST['malamatTB'])).'",
						id_mkec		= "'.trim(mysql_real_escape_string($_POST['id_mkecTB'])).'",
				...
				$sa= '  full_anggota	= "'.trim(mysql_real_escape_string($_POST['full_anggotaTB'])).'",
						nick_anggota	= "'.trim(mysql_real_escape_string($_POST['nick_anggotaTB'])).'",											
						temp_lahir		= "'.trim(mysql_real_escape_string($_POST['temp_lahirTB'])).'",											
						tgl_lahir		= "'.trim(mysql_real_escape_string(tgl_indo3($_POST['tgl_lahirTB']))).'",											
						gol_darah		= "'.trim(mysql_real_escape_string($_POST['gol_darahTB'])).'",											
						jenis_kelamin	= "'.trim(mysql_real_escape_string($_POST['jenis_kelaminTB'])).'",											
						agama			= "'.trim(mysql_real_escape_string($_POST['agamaTB'])).'",											
						status_nikah	= "'.trim(mysql_real_escape_string($_POST['status_nikahTB'])).'",											
						jenis_kecacatan	= "'.trim(mysql_real_escape_string($_POST['jenis_kecacatanTB'])).'",											
						bakat			= "'.trim(mysql_real_escape_string($_POST['bakatTB'])).'",											
						hobi			= "'.trim(mysql_real_escape_string($_POST['hobiTB'])).'",											
						id_mgudep		= "'.trim(mysql_real_escape_string($_POST['mgudepTB'])).'",											
						bahasa			= "'.trim(mysql_real_escape_string($_POST['bahasaTB'])).'"'.$foto;											
				
				$skj= ' nm_perusahaan	= "'.trim(mysql_real_escape_string($_POST['nm_perusahaanTB'])).'",
						bid_usaha		= "'.trim(mysql_real_escape_string($_POST['bid_usahaTB'])).'",
						jabatan			= "'.trim(mysql_real_escape_string($_POST['jabatanTB'])).'",
						alamat_usaha	= "'.trim(mysql_real_escape_string($_POST['alamat_usahaTB'])).'",											
						pendapatan		= "'.trim(mysql_real_escape_string($_POST['pendapatanTB'])).'"';

				$skel= ' nm_ibu	= "'.trim(mysql_real_escape_string($_POST['nm_ibuTB'])).'",
						nm_ayah		= "'.trim(mysql_real_escape_string($_POST['nm_ayahTB'])).'",
						alamat_kel	= "'.trim(mysql_real_escape_string($_POST['alamat_kelTB'])).'",
						telp_kel	= "'.trim(mysql_real_escape_string($_POST['telp_kelTB'])).'",											
						job_ayah	= "'.trim(mysql_real_escape_string($_POST['job_ayahTB'])).'",
						job_ibu		= "'.trim(mysql_real_escape_string($_POST['job_ibuTB'])).'"';

				$ssos= ' ym	= "'.trim(mysql_real_escape_string($_POST['ymTB'])).'",
						gt		= "'.trim(mysql_real_escape_string($_POST['gtTB'])).'",
						msn		= "'.trim(mysql_real_escape_string($_POST['msnTB'])).'",
						skype	= "'.trim(mysql_real_escape_string($_POST['skypeTB'])).'",											
						mirc	= "'.trim(mysql_real_escape_string($_POST['mircTB'])).'",
						twitter	= "'.trim(mysql_real_escape_string($_POST['twitterTB'])).'",
						fb		= "'.trim(mysql_real_escape_string($_POST['fbTB'])).'",
						callsing_orari	= "'.trim(mysql_real_escape_string($_POST['callsing_orariTB'])).'"';

				$sas= ' dasuransi		= "'.trim(mysql_real_escape_string($_POST['dasuransiTB'])).'",
						jenis_asuransi	= "'.trim(mysql_real_escape_string($_POST['jenis_asuransiTB'])).'",
						masa_asuransi	= "'.trim(mysql_real_escape_string($_POST['masa_asuransiTB'])).'",
						kond_kesehatan	= "'.trim(mysql_real_escape_string($_POST['kond_kesehatanTB'])).'"';											
				...
				if ($jumc==0) { //add
					...
					$tipex 	= 'edit';	 
					$s=mysql_fetch_assoc(mysql_query('SELECT id_manggota from manggota where id_mlogin='.$_SESSION['id_mloginp']));
					...
					if (!$exa) {
						...
						if (!$exkj) {
							...
							if (!$exkel) {
								...
								$exsos= mysql_query($sqsos);
								if (!$exsos) {
									$out='{"status":"gagal sosmed"}';
								} else {
									$exas= mysql_query($sqas);
									if (!$exas) {
										// var_dump($sqas);exit();
